...
class EEGSimpleConv(torch.nn.Module):
    def __init__(self, fm, n_convs, resampling, kernel_size,n_chan,n_classes,sfreq,n_subjects=None,isProj=False, *args, **kwargs):
        ...
        self.fc2 = torch.nn.Linear(oldfm, n_subjects) if n_subjects else None #Subject regularization for subject cls
        if isProj:
    ...
    def forward(self, x):
        ...
        for seq in self.blocks:
            y = seq(y)
        y = y.mean(dim = 2)
        proj = self.projection(y)
        out = (self.fc(y),self.fc2(y)) if self.fc2 else self.fc(y)
        return out,y, F.normalize(proj, dim=1)
...
class LinearWithConstraint(nn.Linear):
    ...
    def forward(self, x):
        if self.doWeightNorm:
            # Max-Norm Regularization: overfitting control
            self.weight.data = torch.renorm(
                self.weight.data, p=2, dim=0, maxnorm=self.max_norm
            )
