...
class MovingAvg:
    def __init__(self, network, start):
        ...
        self.network_sma.eval() # only for eval
    ...
    def update_sma(self):
        self.global_iter += 1
        new_dict = {}
        if self.global_iter >= self.sma_start_iter: # start updating sma
            self.sma_count += 1
            for (name, param_q), (_, param_k) in zip(self.network.state_dict().items(),
                                                     self.network_sma.state_dict().items()):
                if 'num_batches_tracked' not in name: # do not update batch norm tracking stats
                    # param_q = (param_k * n + param_q) / (n + 1)
                    new_dict[name] = (
                                (param_k.data.detach().clone() * self.sma_count + param_q.data.detach().clone()) / (
                                    1. + self.sma_count))
        else: # before start, just copy weights
            for (name, param_q), (_, param_k) in zip(self.network.state_dict().items(),
                                                     self.network_sma.state_dict().items()):
