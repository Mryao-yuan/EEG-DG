...
# Dataset IIa of BCI Competition IV
def parseBci42aDataset(datasetPath, savePath,
    ...
    for iSubs, subs in enumerate(subAll): # subjects, test_subjects
        for iSub, sub in enumerate(subs):
            ...
            # filename:s/se + 001~009.mat ==> data:{'x': x, 'y': y, 'c':channels 's': fs}
...
def matToPython(datasetPath, savePath, isFiltered=False):
    ...
    data = []; # [{'fileName':'','data':{'eeg':np.array,'labels':np.array}},...]
    for root, dirs, files in os.walk(datasetPath):
        files = sorted(files) # files:['s001.mat', 's002.mat',...,'se001.mat',...]
        for f in files:
            parD = {}
            parD['fileName'] = f
            parD['data'] = {}
            ...
            if isFiltered:
                ...
                parD['data']['eeg'] = np.transpose(d['x'], (2, 0, 1)).astype('float32') # 3,1000,8->8,3,1000
    ...
    for i, d in enumerate(data):
        ...
        sub = int(d['fileName'][-7:-4])  # subject of the data(1~9)
        sub = str(sub).zfill(3) # ('001'~'009')
        ...
        if len(d['data']['labels']) == 1:
            d['data']['labels'] = np.transpose(d['data']['labels']) #(1,n) -> (n,1)
...
def trasnformAndSave(datasetPath, savePath, transform=None):
    ...
    with open(os.path.join(config['inDatcataPath'], "transform.csv"), 'w') as f:
...
def fetchData(dataFolder, datasetId=0, filterTransform=None):
    r'''
    checks and fetches the data if not present already.
    such as raw mat files, raw python files, multi-view python files.
    raw mat files: from original data save as mat files by preprocessing functions.
    raw python files: mat data to python eegdataset files
    multi-view python files:???
    '''
...
def alignOperation(x, operation=None):
    '''
    function:对齐不同被试/不同时间段的数据分布
    原因:Non-stationary
    表现:空间分布差异,即spatial covariances matrix 差异 [np.matmul(x, x.transpose((0, 2, 1))).mean(0)]描述了不同通道信号之间的相关性
    方法:对数据进行空间对齐,找到变换矩阵 W,将原始数据变化到新的形式,新形式数据的协方差矩阵变为标准矩阵,通过该操作不同的原始数据(subject a,b)的原始协方差不同，但是可以通过变换矩阵W变换到相同的标准协方差矩阵
    $\text{Cov}(\mathbf{X}_{new}) = \text{Cov}(\mathbf{W} \mathbf{X}) = \mathbf{W} \text{Cov}(\mathbf{X}) \mathbf{W}^T = \mathbf{W} \mathbf{R} \mathbf{W}^T \approx \mathbf{I}$
    1) EA:  W = R^(-1/2), R = np.matmul(x, x.transpose((0, 2, 1))).mean(0)
    2) ZCA: W = V * D^(-1/2) * V^T, R = V * D * V^T
    3) SVD: W = U * D^(-1/2) * U^T, R = U
    '''
    # x.shape (b, c, t)
    if operation == 'ea': # Euclidean Alignment
        r = np.matmul(x, x.transpose((0, 2, 1))).mean(0) # 计算空间协方差矩阵 r (c,c)
        r_op = inv(sqrtm(r))
        ...
        x = np.matmul(r_op, x)
    elif operation == 'zca': # ZCA Whitening
