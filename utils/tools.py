...
def get_transform(filtBank=[[4, 8], [8, 12], [12, 16], [16, 20], [20, 24], [24, 28], [28, 32],
    ...
    # theta: 4-8Hz, alpha: 8-12Hz, beta 1: 12-16Hz,...
    # fileterType: 'butter':频率响应平滑、经典, 'cheby2':阻带衰减更强，更陡峭, 'fir':计算量大但相位失真小
    # outputType: 'sos', 'ba'
...
def split_idx(idx, n, seed=0, datasetId=0):
    r'''
    Return two lists of indices corresponding to a random split of the given
    index list, with n portion in the first list and the rest in the last,
    '''
    if datasetId == 100:
        idx1 = idx[:-40]
        idx2 = idx[-40:]
