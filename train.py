...
root = '/home/yaoyuan/EEG/Datasets'
...
def ho(datasetId=None, network=None, numEpochs=200, maxEpochs=200, batchSize=32, feature=32, subTorun=None, ps='',
    ...
    randomFolder = str(time.strftime("%Y-%m-%d--%H-%M-%S", time.localtime())) + str(network) + ps
    ...
    subs = sorted(set([d[3] for d in data.labels])) # count_id, count_id.dat, label, subject, session
    nSub = len(subs)

    if config['subTorun']: # subject list
    ...
    for iSub, sub in enumerate(subs):
        ...
        # remove current one subject for test others for training
        task.remove(sub)
        ...
        for i, t in enumerate(task):
            ...
            TrainTmp = []
            ValTmp = []
            TmpList.append([j for j, d in enumerate(data.labels) if d[3] in t and 0 == d[2]]) # label = 0
            TmpList.append([j for j, d in enumerate(data.labels) if d[3] in t and 1 == d[2]]) # label = 1
            ...
            for eachTask in TmpList:
                ...
                TrainTmp += [item for item in idx1] # 0.8 of leave subject data
                ValTmp += [item for item in idx2]   # 0.2 of leave subject data
            # 每个 subject 的 index 叠加
        ...
        taskValDatasetList = []
        taskTestDatasetList = []
        for i in range(len(TrainIdxList)): # 迭代 subject-1
            trainData = copy.deepcopy(data)
            trainData.createPartialDataset(TrainIdxList[i], loadNonLoadedData=True) # 基于idx文件读取数据
